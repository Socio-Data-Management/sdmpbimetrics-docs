...
The **SDM Funnel** is a custom Power BI visual that displays a multi-step funnel (awareness → consideration → purchase, survey drop-off, conversion pipelines…) with built-in **leakage tracking**, **statistical significance testing against a benchmark segment**, **competitor/other-partition comparison**, and up to **8 extra data columns** per step.
...
| **Leakage tracking** | The drop-off (%) between consecutive steps is computed and shown in a pill at each junction |
| **Significance testing** | A two-proportion z-test flags whether a group's score — or its leakage — is significantly above/below a segment average, at a 90/95/99% confidence level |
...
| **Up to 8 extra columns** | Each step can show extra measures (value, gap vs. the main score, or both), colored by significance, editable via a JSON dialog behind a ⚙ gear icon |
...
| **Full typography control** | Independent font family/size/style for the value, step label, competitor text and column cells |
...
- Survey drop-off analysis with a significance test against a category/segment norm
- Competitive benchmarking: overlay one or two competitor (or "market average") leakage tracks next to your own funnel
- Multi-KPI step tables: add extra columns (NPS, satisfaction, repeat rate…) per step, colored red/green against their own segment average
